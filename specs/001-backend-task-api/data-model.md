...
## Overview
This document defines the data entities and relationships for the backend task management API. The model follows normalization principles and includes proper validation and indexing for optimal performance.

## Entity: User

### Fields
- **id** (UUID, Primary Key)
  - Type: UUID (from JWT token)
  - Constraints: Not null, Unique, Auto-generated
  - Purpose: Unique identifier for each user (derived from JWT token)
...
- **created_at** (DateTime)
  ...
  - Purpose: Timestamp of user record creation
